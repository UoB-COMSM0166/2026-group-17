class ScoreCalculator {
  ...
  calculateExplosionScore(explosion, players, shooterId) {
    ...
    const enemyPts = this.#calculateHit(explosion, enemy, weaponScore);
    const selfPts = this.#calculateHit(explosion, shooter, weaponScore);
  ...
  #calculateHit(explosion, cannon, weaponScore) {
    ...
    const basePower = weaponScore[explosion.kind] || 100;
    const base = Math.max(0, map(distance, 0, explosion.maxRadius, basePower * 2, 0));
